...
class dataActions extends sfActions {
	...
	$excel = $this->executeExcel($request, $this->indicatorObjects, $this->years, $this->name);
    }
    ...
    public function executeCustomShow(sfWebRequest $request) {
        ...
        $this->years = $request->getParameter('years');
	//check for correct request, go back to index if not
        if( $request->getMethod() != "POST") {
            $this->forward("data", "customIndex");
        }
        $this->indicatorClassnames = $request->getParameter('data');
        ...
        $this->imagePath = sfConfig::get("app_graphUrl");
        //random filename for excel and png files to be created
        $this->name = $this->createRandomFileName();
        ...
        $excel = $this->executeExcel($request,
                $this->indicatorObjects,
                $this->years,
                $this->name);
    }
...
public function executeExcel(sfWebRequest $request,
        ...
        foreach($indicatorObjects as $indicatorObject) {
            $objPHPExcel->getActiveSheet()->getStyle($column[$columnCount].$rowCount)->getFont()->setBold(true);
            $objPHPExcel->getActiveSheet()->setCellValue($column[$columnCount].$rowCount, $indicatorObject->entity->getName());
            $rowCount++;
            $columnCount = $columnCount +1;
            $objPHPExcel->getActiveSheet()->getStyle($column[$columnCount].$rowCount)->getFont()->setBold(true);
            $objPHPExcel->getActiveSheet()->setCellValue($column[$columnCount].$rowCount, $indicatorObject->getName()." ".$indicatorObject->getUnitTitle());
            $rowCount++;
            $columnCount -= 1;
            $yearValueArray = $indicatorObject->getValues();
            foreach($years as $year) {
                ...
                $rowCount++;
                $objPHPExcel->getActiveSheet()->setCellValue($column[$columnCount].$rowCount,
                        strip_tags($indicatorObject->getDescription()));
                $columnCount--;
            }
            $rowCount = $rowCount +2;
